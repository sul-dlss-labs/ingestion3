...
/**
  ...
  *   1)  --output  A base path to save the harvest, mapping, enrichment, json-l and report outputs
  *   2)  --conf    A path to the application configuration file
  *   3)  --name    Provider short name
  */
object Ingest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Outputs
    val harvestDataOut = baseDataOut+"/harvest"
    ...
    // Harvest
    logger.info(s"Saving harvest output to: $harvestDataOut")
    executeHarvest(shortName, harvestDataOut, conf, logger)
